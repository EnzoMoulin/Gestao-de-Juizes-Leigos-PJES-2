...
const headers={...run('JL_CONFIG.HEADERS'),REQUEST_QUANTITY:run('JL_CONFIG.REQUEST_QUANTITY_HEADER')}, titles=Object.values(headers);
...
function reset(){writes=0;locked=false;active='admin@tjes.jus.br';sheets={
 'Respostas ao formulário 1':new Sheet('Respostas ao formulário 1',[titles.slice(),
 row({FUNCTION:'Assessor',NAME:'Pedido encerrado',EMAIL:'viewer@tjes.jus.br',CAPACITY:'999',REQUEST_QUANTITY:'20',STATUS:'Concluído',ASSIGNED_JUDGE:'Juíza Teste',NOTES:'Encerrado para teste'}),
 row({FUNCTION:'Juíza Leiga',NAME:'Juíza Teste',CAPACITY:'100',STATUS:'Pendente'}),
 row({FUNCTION:'Assessor',NAME:'Pedido ativo',EMAIL:'outro@tjes.jus.br',CAPACITY:'888',REQUEST_QUANTITY:'90',STATUS:'Em atendimento',ASSIGNED_JUDGE:'Juíza Teste'})]),
...
test('quantidade ambígua bloqueia designação',()=>{sheets['Respostas ao formulário 1'].rows[1][column('REQUEST_QUANTITY')]='20 a 30';assert.throws(()=>assign(current().versao,'Motivo válido',true),/inteira positiva/);assert.equal(writes,0);});
test('carga desconhecida não cria disponibilidade fictícia',()=>{sheets['Respostas ao formulário 1'].rows[3][column('REQUEST_QUANTITY')]='indefinida';assert.equal(data().metricas.disponiveis,0);assert.equal(data().juizes[0].disponiveis,null);assert.throws(()=>assign(current().versao),/quantidade inválida/);assert.equal(writes,0);});
test('quantidade ausente em solicitação antiga deixa carga parcial sem invalidar capacidade do juiz',()=>{sheets['Respostas ao formulário 1'].rows[3][column('REQUEST_QUANTITY')]='';const juiz=data().juizes[0];assert.equal(juiz.requerRevisao,false);assert.equal(juiz.cargaParcial,true);assert.equal(JSON.stringify(juiz.quantidadesAtribuidasAusentes),'[4]');assert.equal(juiz.designadas,0);assert.equal(juiz.disponiveis,100);assert.equal(data().metricas.disponiveis,1);});
...
test('redesignar para o mesmo juiz não duplica carga existente',()=>{const source=sheets['Respostas ao formulário 1'];source.rows[1][column('STATUS')]='Em atendimento';sheets['Respostas ao formulário 1'].rows[3][column('REQUEST_QUANTITY')]=80;assign(current().versao);assert.equal(data().juizes[0].designadas,100);});
...
test('quantidade vem do formulário, ignora gestão legada e preserva F ao salvar',()=>{sheets.GESTAO_SOLICITACOES.rows[1][5]=777;assert.equal(current().quantidadeNumerica,20);update(current().versao,'Concluído');assert.equal(current().quantidadeNumerica,20);assert.equal(sheets.GESTAO_SOLICITACOES.rows[1][5],777);assert.equal(sheets['Respostas ao formulário 1'].rows[1][column('CAPACITY')],'999');});
test('API rejeita quantidade administrativa antes de qualquer escrita',()=>{for(const value of [1,20,'20','',null,{},true]){assert.throws(()=>assign(current().versao,'Motivo válido',true,value),/não pode ser alterada/);assert.throws(()=>update(current().versao,'Concluído','',false,value),/não pode ser alterada/);assert.equal(writes,0);}});
test('designação usa quantidade da origem e não grava quantidade na gestão',()=>{sheets['Respostas ao formulário 1'].rows[1][column('REQUEST_QUANTITY')]=10;sheets.GESTAO_SOLICITACOES.rows[1][5]=777;assign(current().versao);assert.equal(current().quantidadeNumerica,10);assert.equal(current().status,'Em atendimento');assert.equal(data().juizes[0].designadas,100);assert.equal(sheets.GESTAO_SOLICITACOES.rows[1][5],777);});
test('quantidade ausente nunca usa capacidade, gestão ou contagem de processos',()=>{const source=sheets['Respostas ao formulário 1'];source.rows[1][column('REQUEST_QUANTITY')]='';source.rows[1][column('CASES')]='123\n456';assert.equal(current().quantidadeInformada,false);assert.equal(current().quantidadeValida,false);assert.throws(()=>assign(current().versao,'Motivo válido',true),/solicitante/);assert.throws(()=>update(current().versao,'Em atendimento','Motivo válido',true),/solicitante/);assert.equal(writes,0);update(current().versao,'Cancelado');assert.equal(current().status,'Cancelado');});
test('ausência do cabeçalho permite leitura e encerramento, mas não designação',()=>{sheets['Respostas ao formulário 1'].rows[0][column('REQUEST_QUANTITY')]='Outro campo';assert.equal(data().fonte.quantidadeFormularioConfigurada,false);assert.equal(current().quantidadeInformada,false);assert.throws(()=>assign(current().versao,'Motivo válido',true),/solicitante/);assert.equal(writes,0);update(current().versao,'Cancelado');});
test('alteração de resposta pelo solicitante invalida a versão aberta',()=>{const old=current().versao;sheets['Respostas ao formulário 1'].rows[1][column('REQUEST_QUANTITY')]=10;assert.throws(()=>assign(old),/desatualizada/);assert.equal(writes,0);assign(current().versao);assert.equal(data().juizes[0].designadas,100);});
test('quantidade é localizada por título, não por posição',()=>{const source=sheets['Respostas ao formulário 1'];source.rows=source.rows.map(r=>[r.at(-1),...r.slice(0,-1)]);assert.equal(current().quantidadeNumerica,20);assert.equal(data().fonte.quantidadeFormularioConfigurada,true);});
test('schema antigo com cinco colunas de gestão permanece utilizável',()=>{sheets.GESTAO_SOLICITACOES.rows=sheets.GESTAO_SOLICITACOES.rows.map(r=>r.slice(0,5));assert.equal(current().quantidadeNumerica,20);update(current().versao,'Cancelado');assert.equal(current().status,'Cancelado');});

test('concluir e cancelar aceitam observações vazias e registram auditoria',()=>{for(const status of ['Concluído','Cancelado']){reset();ctx.closeVersion=current().versao;ctx.closeStatus=status;run('apiAtualizarSolicitacao("token",2,closeStatus,"","Normal","",closeVersion,"",false)');assert.equal(current().status,status);assert.equal(current().observacoes,'');assert.equal(JSON.parse(sheets.AUDITORIA.rows.at(-1)[6]).observacoes,'');}});
test('cadastro expõe carimbo original sem inventar data ausente',()=>{assert.equal(data().juizes[0].data,'');assert.equal(data().juizes[0].dataIso,'');sheets['Respostas ao formulário 1'].rows[2][column('TIMESTAMP')]='14/09/2026 10:25:30';assert.equal(data().juizes[0].data,'14/09/2026 10:25:30');assert.equal(run('apiListarCadastrosJuizes("token")[0].data'),'14/09/2026 10:25:30');});
