...
function update(version,status='Em atendimento',reason='',confirm=false){ctx.args=[version,status,reason,confirm];return run('apiAtualizarSolicitacao("token",2,args[1],"Andamento atualizado","Alta","",args[0],args[2],args[3])');}
function assign(version,reason='',confirm=false){ctx.args=[version,reason,confirm];return run('apiDesignarJuiz("token",2,"Juíza Teste",args[1],args[2],"Normal","",args[0])');}
let passed=0;
...
test('alteração de prioridade ou prazo invalida a versão',()=>{const old=current().versao;sheets.GESTAO_SOLICITACOES.rows.push([2,'Urgente','2026-09-20','revisao','admin@tjes.jus.br']);assert.throws(()=>update(old,'Cancelado'),/desatualizada/);assert.equal(writes,0);});
...
test('quantidade ambígua bloqueia designação',()=>{sheets['Respostas ao formulário 1'].rows[1][column('CAPACITY')]='20 a 30';assert.throws(()=>assign(current().versao,'Motivo válido',true),/inteiro positivo/);assert.equal(writes,0);});
test('carga desconhecida não cria disponibilidade fictícia',()=>{sheets['Respostas ao formulário 1'].rows[3][column('CAPACITY')]='indefinida';assert.equal(data().metricas.disponiveis,0);assert.equal(data().juizes[0].disponiveis,null);assert.throws(()=>assign(current().versao),/quantidade inválida/);assert.equal(writes,0);});
test('quantidade ausente em solicitação antiga deixa carga parcial sem invalidar capacidade do juiz',()=>{sheets['Respostas ao formulário 1'].rows[3][column('CAPACITY')]='';const juiz=data().juizes[0];assert.equal(juiz.requerRevisao,false);assert.equal(juiz.cargaParcial,true);assert.equal(JSON.stringify(juiz.quantidadesAtribuidasAusentes),'[4]');assert.equal(juiz.designadas,0);assert.equal(juiz.disponiveis,100);assert.equal(data().metricas.disponiveis,1);});
...
test('redesignar para o mesmo juiz não duplica carga existente',()=>{const source=sheets['Respostas ao formulário 1'];source.rows[1][column('STATUS')]='Em atendimento';source.rows[3][column('CAPACITY')]='80';assign(current().versao);assert.equal(data().juizes[0].designadas,100);});
