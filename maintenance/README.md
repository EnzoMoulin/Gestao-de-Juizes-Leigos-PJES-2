...
5. O comando verifica os cabeçalhos de origem e prepara USUARIOS, AUDITORIA e GESTAO_SOLICITACOES sem apagar respostas ou usuários. A coluna `QUANTIDADE_MINUTAS` é histórica, fica preservada e não é mais utilizada. A quantidade vem da nova pergunta obrigatória do formulário para magistrados/assessores; veja [configuração e ativação](../docs/QUANTIDADE_SOLICITANTE.md). Este comando não cria perguntas no Google Forms.
6. Se existir a aba `Juízes Leigos Disponíveis`, o comando alinha somente sua linha de cabeçalho à ordem retornada pela fórmula `FILTER` (A:R). Se a cópia possuir uma tabela nativa com uma coluna excedente, essa coluna final recebe `COLUNA_AUXILIAR_LEGADA`, pois tabelas não aceitam cabeçalho vazio. A fórmula e as respostas originais permanecem intactas.
